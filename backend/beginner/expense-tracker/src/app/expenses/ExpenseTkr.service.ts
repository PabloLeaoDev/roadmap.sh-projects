import * as ExpenseTkrModel from './ExpenseTkr.model';
import { ResponseCli } from '../../interfaces/generic.interface';
import { UpdatableFlags } from '../../interfaces/expense.interface';

export async function toAddExpense(args: string[]): Promise<ResponseCli> {
  try {
...
export async function toUpdateExpense(args: string[]): Promise<ResponseCli> {
  try {
...
export async function toListAllExpenses(): Promise<ResponseCli | void> {
  try {
...
export async function toListExpenseWithFilter(args: string[]): Promise<ResponseCli | void> {
  try {
...
export async function toDeleteExpenses(args: string[]): Promise<ResponseCli | void> {
  try {
    const id: number | undefined = Number((args[1]));

    if (args.length < 1 && args.length > 2) throw new Error('You must set at most one argument to delete option');

    await ExpenseTkrModel.deleteExpenses(id);
  } catch (error) {
    return { success: false, message: (error as Error).message };
  }
}